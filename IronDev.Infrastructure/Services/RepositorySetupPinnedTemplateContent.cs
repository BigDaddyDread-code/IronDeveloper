...
internal static class RepositorySetupPinnedTemplateContent
{
    public const string Solution =
        "<Solution>\n" +
        "  <Project Path=\"src/{{APP_PROJECT_NAME}}/{{APP_PROJECT_NAME}}.csproj\" />\n" +
        "  <Project Path=\"tests/{{TEST_PROJECT_NAME}}/{{TEST_PROJECT_NAME}}.csproj\" />\n" +
        "</Solution>\n";

    public const string ApplicationPackagesLock =
        "{\n" +
        "  \"version\": 1,\n" +
        "  \"dependencies\": {\n" +
        "    \"net10.0-windows7.0\": {}\n" +
    ...
    // Raw string literals inherit the checkout's source-file line endings. Normalize this
    // pinned authority at its source so the canonical bundle remains LF-only on Windows
    // and Unix without weakening the bundle codec's rejection of arbitrary CR content.
    public static string TestPackagesLock { get; } = ToLf("""
{
  "version": 1,
  "dependencies": {
    "net10.0-windows7.0": {
      "{{APP_PROJECT_NAME_LOWER}}": {
        "type": "Project"
      },
      "MSTest": {
        "type": "Direct",
...
""" + "\n");
    ...
    private static string ToLf(string value) => value
