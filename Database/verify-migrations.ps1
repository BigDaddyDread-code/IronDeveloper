...
try {
    ...
    try {
        $checks = @(
            ...
            ,@{ Name = "ProjectProfiles checked project scope foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id = OBJECT_ID(N'dbo.ProjectProfiles') AND name = N'FK_ProjectProfiles_ProjectScope' AND is_not_trusted = 0" }
            ,@{ Name = "ProjectCommands checked project scope foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id = OBJECT_ID(N'dbo.ProjectCommands') AND name = N'FK_ProjectCommands_ProjectScope' AND is_not_trusted = 0" }
            ,@{ Name = "Runs checked ticket scope foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id = OBJECT_ID(N'dbo.Runs') AND name = N'FK_Runs_ProjectTickets' AND is_not_trusted = 0" }
            ,@{ Name = "Runs ticket requires project constraint"; Sql = "SELECT COUNT(*) FROM sys.check_constraints WHERE parent_object_id = OBJECT_ID(N'dbo.Runs') AND name = N'CK_Runs_TicketRequiresProject' AND is_not_trusted = 0" }
            ,@{ Name = "Runs retention index"; Sql = "SELECT COUNT(*) FROM sys.indexes WHERE object_id = OBJECT_ID(N'dbo.Runs') AND name = N'IX_Runs_State_UpdatedUtc'" }
            ,@{ Name = "ProjectTickets source chat foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id = OBJECT_ID(N'dbo.ProjectTickets') AND name = N'FK_ProjectTickets_SourceChatMessage' AND is_not_trusted = 0" }
            ,@{ Name = "ProjectTickets source document foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id = OBJECT_ID(N'dbo.ProjectTickets') AND name = N'FK_ProjectTickets_SourceDocumentVersion' AND is_not_trusted = 0" }
            ,@{ Name = "ProjectTickets status retrieval index"; Sql = "SELECT COUNT(*) FROM sys.indexes WHERE object_id = OBJECT_ID(N'dbo.ProjectTickets') AND name = N'IX_ProjectTickets_Status_CreatedDate'" }
            ,@{ Name = "Semantic artefact checked project scope foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id = OBJECT_ID(N'dbo.SemanticArtefacts') AND name = N'FK_SemanticArtefacts_ProjectScope' AND is_not_trusted = 0" }
            ,@{ Name = "Embedding job checked project scope foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id = OBJECT_ID(N'dbo.EmbeddingJobs') AND name = N'FK_EmbeddingJobs_ProjectScope' AND is_not_trusted = 0" }
            ,@{ Name = "Semantic trace retrieval index"; Sql = "SELECT COUNT(*) FROM sys.indexes WHERE object_id = OBJECT_ID(N'dbo.SemanticSearchTraces') AND name = N'IX_SemanticSearchTraces_Project_CreatedUtc'" }
            ,@{ Name = "Semantic index run status index"; Sql = "SELECT COUNT(*) FROM sys.indexes WHERE object_id = OBJECT_ID(N'dbo.SemanticIndexRuns') AND name = N'IX_SemanticIndexRuns_Status_StartedAtUtc'" }
            ,@{ Name = "Work Item supersession index"; Sql = "SELECT COUNT(*) FROM sys.indexes WHERE object_id = OBJECT_ID(N'dbo.WorkItemContracts') AND name = N'IX_WorkItemContracts_Supersedes'" }
            ,@{ Name = "Agent handoff checked supersession foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id = OBJECT_ID(N'a2a.AgentHandoff') AND name = N'FK_AgentHandoff_Supersedes' AND is_not_trusted = 0" }
            ,@{ Name = "Decision category defaults"; Sql = "SELECT CASE WHEN COUNT(*) = 10 AND MIN(SortOrder) = 1 AND MAX(SortOrder) = 10 THEN 1 ELSE 0 END FROM dbo.DecisionCategories WHERE Name IN (N'Architecture', N'Code Standards', N'Product', N'Data', N'Infrastructure', N'AI / Prompting', N'UX / UI', N'Workflow / Process', N'Integration', N'Security')" }
            ...
            ,@{ Name = "Current memory index lifecycle view"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.vw_CurrentMemoryIndexLifecycle', N'V') IS NULL THEN 0 ELSE 1 END" }
             ...
             ,@{ Name = "Workbench session active chat binding"; Sql = "SELECT CASE WHEN COL_LENGTH(N'dbo.WorkbenchSessions', N'ActiveChatSessionId') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Workbench session same-project chat foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.WorkbenchSessions') AND name=N'FK_WorkbenchSessions_ActiveChatSession' AND is_not_trusted=0" }
             ,@{ Name = "Workbench agent-run exact chat binding foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.WorkbenchAgentRuns') AND name=N'FK_WorkbenchAgentRuns_ExactChatBinding' AND is_not_trusted=0" }
             ,@{ Name = "Workbench agent-run active slot constraint"; Sql = "SELECT COUNT(*) FROM sys.check_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.WorkbenchAgentRuns') AND name=N'CK_WorkbenchAgentRuns_ActiveRunSlot'" }
             ,@{ Name = "Workbench agent-run one-active-session index"; Sql = "SELECT COUNT(*) FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.WorkbenchAgentRuns') AND name=N'UX_WorkbenchAgentRuns_ActiveSession' AND is_unique=1 AND has_filter=1" }
             ,@{ Name = "Workbench agent-run context format pair constraint"; Sql = "SELECT COUNT(*) FROM sys.check_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.WorkbenchAgentRuns') AND name=N'CK_WorkbenchAgentRuns_ContextFormatVersionPair'" }
             ...
             ,@{ Name = "Workbench agent-run attempt outcome constraint"; Sql = "SELECT COUNT(*) FROM sys.check_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.WorkbenchAgentRunAttempts') AND name=N'CK_WorkbenchAgentRunAttempts_Outcome'" }
             ,@{ Name = "Workbench agent-run attempt completion constraint"; Sql = "SELECT COUNT(*) FROM sys.check_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.WorkbenchAgentRunAttempts') AND name=N'CK_WorkbenchAgentRunAttempts_Completion'" }
             ,@{ Name = "Workbench agent-run one-unfinished-attempt index"; Sql = "SELECT COUNT(*) FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.WorkbenchAgentRunAttempts') AND name=N'UX_WorkbenchAgentRunAttempts_Unfinished' AND is_unique=1 AND has_filter=1" }
             ...
             ,@{ Name = "Workbench agent-run version columns"; Sql = "SELECT CASE WHEN COL_LENGTH(N'dbo.WorkbenchAgentRuns', N'AgentVersion') IS NULL OR COL_LENGTH(N'dbo.WorkbenchAgentRuns', N'PromptVersion') IS NULL OR COL_LENGTH(N'dbo.WorkbenchAgentRuns', N'ToolPolicyVersion') IS NULL OR COL_LENGTH(N'dbo.WorkbenchAgentRuns', N'ContextSchemaVersion') IS NULL OR COL_LENGTH(N'dbo.WorkbenchAgentRuns', N'ContextCanonicalizationVersion') IS NULL OR COL_LENGTH(N'dbo.WorkbenchAgentRuns', N'OutputSchemaVersion') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Workbench BA preparation table"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.WorkbenchBusinessAnalystPreparations', N'U') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Workbench BA tool-call audit table"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.WorkbenchBusinessAnalystToolCallAudits', N'U') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Workbench BA exact preparation-attempt foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.WorkbenchBusinessAnalystPreparations') AND name=N'FK_WorkbenchBusinessAnalystPreparations_ExactAttempt' AND is_not_trusted=0" }
             ,@{ Name = "Workbench BA exact tool-preparation foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.WorkbenchBusinessAnalystToolCallAudits') AND name=N'FK_WorkbenchBusinessAnalystToolCallAudits_ExactPreparation' AND is_not_trusted=0" }
             ,@{ Name = "Workbench BA one preparation per attempt"; Sql = "SELECT COUNT(*) FROM sys.key_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.WorkbenchBusinessAnalystPreparations') AND name=N'UQ_WorkbenchBusinessAnalystPreparations_Attempt'" }
             ,@{ Name = "Workbench BA one audit per preparation tool"; Sql = "SELECT COUNT(*) FROM sys.key_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.WorkbenchBusinessAnalystToolCallAudits') AND name=N'UQ_WorkbenchBusinessAnalystToolCallAudits_PreparationTool'" }
             ,@{ Name = "Workbench BA tool-call status constraint"; Sql = "SELECT COUNT(*) FROM sys.check_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.WorkbenchBusinessAnalystToolCallAudits') AND name=N'CK_WorkbenchBusinessAnalystToolCallAudits_Status'" }
             ,@{ Name = "Workbench BA preparation append-only trigger"; Sql = "SELECT COUNT(*) FROM sys.triggers WHERE object_id=OBJECT_ID(N'dbo.TR_WorkbenchBusinessAnalystPreparations_BlockUpdateDelete', N'TR') AND is_disabled=0" }
             ,@{ Name = "Workbench BA tool-call append-only trigger"; Sql = "SELECT COUNT(*) FROM sys.triggers WHERE object_id=OBJECT_ID(N'dbo.TR_WorkbenchBusinessAnalystToolCallAudits_BlockUpdateDelete', N'TR') AND is_disabled=0" }
             ,@{ Name = "Workbench BA invocation audit table"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.WorkbenchBusinessAnalystInvocationAudits', N'U') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Workbench BA exact invocation-preparation foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.WorkbenchBusinessAnalystInvocationAudits') AND name=N'FK_WorkbenchBusinessAnalystInvocationAudits_ExactPreparation' AND is_not_trusted=0" }
             ,@{ Name = "Workbench BA one invocation per preparation"; Sql = "SELECT COUNT(*) FROM sys.key_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.WorkbenchBusinessAnalystInvocationAudits') AND name=N'UQ_WorkbenchBusinessAnalystInvocationAudits_Preparation'" }
             ,@{ Name = "Workbench BA invocation usage constraint"; Sql = "SELECT COUNT(*) FROM sys.check_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.WorkbenchBusinessAnalystInvocationAudits') AND name=N'CK_WorkbenchBusinessAnalystInvocationAudits_Usage'" }
             ,@{ Name = "Workbench BA invocation append-only trigger"; Sql = "SELECT COUNT(*) FROM sys.triggers WHERE object_id=OBJECT_ID(N'dbo.TR_WorkbenchBusinessAnalystInvocationAudits_BlockUpdateDelete', N'TR') AND is_disabled=0" }
             ,@{ Name = "Project understanding document schema column"; Sql = "SELECT CASE WHEN COL_LENGTH(N'dbo.ProjectUnderstandings', N'DocumentSchemaVersion') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Project understanding AgentRun foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.ProjectUnderstandings') AND name=N'FK_ProjectUnderstandings_AgentRun' AND is_not_trusted=0" }
             ...
             ,@{ Name = "Ticket proposal set table"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.TicketProposalSets', N'U') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Ticket proposal revision table"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.TicketProposalSetRevisions', N'U') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Ticket proposal exact fence foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.TicketProposalSets') AND name=N'FK_TicketProposalSets_ExactFence' AND is_not_trusted=0" }
             ,@{ Name = "Ticket proposal revision project-scoped uniqueness"; Sql = "SELECT COUNT(*) FROM sys.key_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.TicketProposalSetRevisions') AND name=N'UQ_TicketProposalSetRevisions_ProjectSetRevision'" }
             ,@{ Name = "Ticket proposal exact understanding foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.TicketProposalSets') AND name=N'FK_TicketProposalSets_Understanding' AND is_not_trusted=0" }
             ,@{ Name = "Ticket proposal revision project-scoped parent foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.TicketProposalSetRevisions') AND name=N'FK_TicketProposalSetRevisions_Set' AND is_not_trusted=0" }
             ,@{ Name = "Ticket proposal append-only trigger"; Sql = "SELECT COUNT(*) FROM sys.triggers WHERE parent_id=OBJECT_ID(N'dbo.TicketProposalSetRevisions') AND name=N'trg_TicketProposalSetRevisions_AppendOnly'" }
             ,@{ Name = "Workbench proposal-purpose columns"; Sql = "SELECT CASE WHEN COL_LENGTH(N'dbo.WorkbenchAgentRuns', N'InvocationKind') IS NULL OR COL_LENGTH(N'dbo.WorkbenchAgentRuns', N'TicketInstruction') IS NULL OR COL_LENGTH(N'dbo.WorkbenchAgentRuns', N'TicketProposalSetId') IS NULL OR COL_LENGTH(N'dbo.WorkbenchAgentRuns', N'TicketProposalRevision') IS NULL OR COL_LENGTH(N'dbo.WorkbenchAgentRuns', N'MaterializedTicketProposalRevision') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Ticket proposal committed-state columns"; Sql = "SELECT CASE WHEN COL_LENGTH(N'dbo.TicketProposalSets', N'CommittedRevision') IS NULL OR COL_LENGTH(N'dbo.TicketProposalSets', N'CommittedByActorUserId') IS NULL OR COL_LENGTH(N'dbo.TicketProposalSets', N'CommittedByClientOperationId') IS NULL OR COL_LENGTH(N'dbo.TicketProposalSets', N'CommittedAtUtc') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Ticket proposal committed status constraint"; Sql = "SELECT COUNT(*) FROM sys.check_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.TicketProposalSets') AND name=N'CK_TicketProposalSets_Status' AND definition LIKE N'%Committed%' AND is_not_trusted=0" }
             ,@{ Name = "Ticket proposal committed revision change constraint"; Sql = "SELECT COUNT(*) FROM sys.check_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.TicketProposalSetRevisions') AND name=N'CK_TicketProposalSetRevisions_ChangeKind' AND definition LIKE N'%Committed%' AND is_not_trusted=0" }
             ,@{ Name = "ProjectTickets checked project scope foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.ProjectTickets') AND name=N'FK_ProjectTickets_ProjectScope' AND is_not_trusted=0" }
             ,@{ Name = "ProjectTickets tenant-project-ticket candidate key"; Sql = "SELECT COUNT(*) FROM sys.key_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.ProjectTickets') AND name=N'UQ_ProjectTickets_TenantProjectId'" }
             ,@{ Name = "Ticket proposal commitment table"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.TicketProposalCommitments', N'U') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Ticket proposal commitment-ticket map"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.TicketProposalCommitmentTickets', N'U') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Ticket proposal commitment dependency map"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.TicketProposalCommitmentDependencies', N'U') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Ticket proposal commitment exact reviewed revision foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.TicketProposalCommitments') AND name=N'FK_TicketProposalCommitments_ReviewedRevision' AND is_not_trusted=0" }
             ,@{ Name = "Ticket proposal commitment exact fence foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.TicketProposalCommitments') AND name=N'FK_TicketProposalCommitments_ExactFence' AND is_not_trusted=0" }
             ,@{ Name = "Ticket proposal exact project-ticket mapping foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.TicketProposalCommitmentTickets') AND name=N'FK_TicketProposalCommitmentTickets_ProjectTicket' AND is_not_trusted=0" }
             ,@{ Name = "Ticket proposal exact dependency mapping foreign keys"; Sql = "SELECT CASE WHEN COUNT(*)=2 THEN 1 ELSE 0 END FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.TicketProposalCommitmentDependencies') AND name IN (N'FK_TicketProposalCommitmentDependencies_DependentMapping', N'FK_TicketProposalCommitmentDependencies_DependsOnMapping') AND is_not_trusted=0" }
             ,@{ Name = "Ticket proposal commitment append-only triggers"; Sql = "SELECT CASE WHEN COUNT(*)=3 THEN 1 ELSE 0 END FROM sys.triggers WHERE parent_id IN (OBJECT_ID(N'dbo.TicketProposalCommitments'), OBJECT_ID(N'dbo.TicketProposalCommitmentTickets'), OBJECT_ID(N'dbo.TicketProposalCommitmentDependencies')) AND name IN (N'trg_TicketProposalCommitments_AppendOnly', N'trg_TicketProposalCommitmentTickets_AppendOnly', N'trg_TicketProposalCommitmentDependencies_AppendOnly') AND is_disabled=0" }
             ...
             ,@{ Name = "Sandbox qualification attempt table"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.SandboxQualificationAttempts', N'U') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Sandbox evidence manifest table"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.SandboxEvidenceManifests', N'U') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Sandbox client-operation result columns"; Sql = "SELECT CASE WHEN COL_LENGTH(N'dbo.ClientOperations', N'ResultSandboxQualificationAttemptId') IS NULL OR COL_LENGTH(N'dbo.ClientOperations', N'ResultSandboxEvidenceManifestId') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Sandbox exact client-operation authority foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.SandboxQualificationAttempts') AND name=N'FK_SandboxQualificationAttempts_ClientOperationAuthority' AND is_not_trusted=0" }
             ,@{ Name = "Sandbox exact Workbench fence foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.SandboxQualificationAttempts') AND name=N'FK_SandboxQualificationAttempts_Fence' AND is_not_trusted=0" }
             ,@{ Name = "Sandbox exact provisioning source authority foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.SandboxQualificationAttempts') AND name=N'FK_SandboxQualificationAttempts_SourceAuthority' AND is_not_trusted=0" }
             ,@{ Name = "Sandbox durable recovery scheduling column"; Sql = "SELECT CASE WHEN COL_LENGTH(N'dbo.SandboxQualificationAttempts', N'LastRecoveryAttemptAtUtc') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Sandbox durable trusted-supervisor authority columns"; Sql = "SELECT CASE WHEN EXISTS (SELECT 1 FROM sys.columns WHERE object_id=OBJECT_ID(N'dbo.SandboxQualificationAttempts') AND name=N'TrustedSupervisorVersion' AND is_nullable=0 AND max_length=200) AND EXISTS (SELECT 1 FROM sys.columns WHERE object_id=OBJECT_ID(N'dbo.SandboxQualificationAttempts') AND name=N'TrustedSupervisorSha256' AND is_nullable=0 AND max_length=64) THEN 1 ELSE 0 END" }
             ,@{ Name = "Sandbox trusted-supervisor version constraint"; Sql = "SELECT COUNT(*) FROM sys.check_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.SandboxQualificationAttempts') AND name=N'CK_SandboxQualificationAttempts_TrustedSupervisorVersion' AND is_not_trusted=0" }
             ,@{ Name = "Sandbox trusted-supervisor hash constraint"; Sql = "SELECT COUNT(*) FROM sys.check_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.SandboxQualificationAttempts') AND name=N'CK_SandboxQualificationAttempts_TrustedSupervisorSha256' AND is_not_trusted=0" }
             ,@{ Name = "Sandbox exact evidence authority foreign key"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.SandboxEvidenceManifests') AND name=N'FK_SandboxEvidenceManifests_AttemptAuthority' AND is_not_trusted=0" }
             ,@{ Name = "Sandbox actor-scoped operation key"; Sql = "SELECT CASE WHEN EXISTS (SELECT 1 FROM sys.key_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.SandboxQualificationAttempts') AND name=N'UQ_SandboxQualificationAttempts_ClientOperationActor') AND (SELECT COUNT(*) FROM sys.index_columns ic INNER JOIN sys.columns c ON c.object_id=ic.object_id AND c.column_id=ic.column_id WHERE ic.object_id=OBJECT_ID(N'dbo.SandboxQualificationAttempts') AND ic.index_id=(SELECT unique_index_id FROM sys.key_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.SandboxQualificationAttempts') AND name=N'UQ_SandboxQualificationAttempts_ClientOperationActor') AND ic.key_ordinal>0 AND c.name IN (N'TenantId',N'ProjectId',N'ActorUserId',N'ClientOperationId'))=4 THEN 1 ELSE 0 END" }
             ,@{ Name = "Sandbox actor-scoped ClientOperations index"; Sql = "SELECT CASE WHEN EXISTS (SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.ClientOperations') AND name=N'UX_ClientOperations_SandboxQualificationProjectOperation' AND is_unique=1 AND has_filter=1) AND (SELECT COUNT(*) FROM sys.index_columns ic INNER JOIN sys.columns c ON c.object_id=ic.object_id AND c.column_id=ic.column_id WHERE ic.object_id=OBJECT_ID(N'dbo.ClientOperations') AND ic.index_id=INDEXPROPERTY(OBJECT_ID(N'dbo.ClientOperations'), N'UX_ClientOperations_SandboxQualificationProjectOperation', 'IndexId') AND ic.key_ordinal>0 AND c.name IN (N'TenantId',N'ActorUserId',N'OperationKind',N'ResourceScopeId',N'ClientOperationId'))=5 THEN 1 ELSE 0 END" }
             ,@{ Name = "Sandbox one-running-attempt index"; Sql = "SELECT COUNT(*) FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.SandboxQualificationAttempts') AND name=N'UX_SandboxQualificationAttempts_OneRunningPerProject' AND is_unique=1 AND has_filter=1 AND filter_definition LIKE N'%State%Running%'" }
             ,@{ Name = "Sandbox attempt completion constraint"; Sql = "SELECT COUNT(*) FROM sys.check_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.SandboxQualificationAttempts') AND name=N'CK_SandboxQualificationAttempts_Completion' AND is_not_trusted=0" }
             ,@{ Name = "Sandbox evidence append-only trigger"; Sql = "SELECT COUNT(*) FROM sys.triggers WHERE parent_id=OBJECT_ID(N'dbo.SandboxEvidenceManifests') AND name=N'TR_SandboxEvidenceManifests_AppendOnly' AND is_disabled=0" }
             ,@{ Name = "Sandbox terminal attempt immutability trigger"; Sql = "SELECT COUNT(*) FROM sys.triggers WHERE parent_id=OBJECT_ID(N'dbo.SandboxQualificationAttempts') AND name=N'TR_SandboxQualificationAttempts_TerminalImmutable' AND is_disabled=0 AND OBJECT_DEFINITION(object_id) LIKE N'%UPDATE(TrustedSupervisorVersion)%' AND OBJECT_DEFINITION(object_id) LIKE N'%UPDATE(TrustedSupervisorSha256)%'" }
             ,@{ Name = "Technical validation attempt table"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.TechnicalValidationAttempts', N'U') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Repository state observation table"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.RepositoryStateObservations', N'U') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Build validation record table"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.BuildValidationRecords', N'U') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Test validation record table"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.TestValidationRecords', N'U') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Code index snapshot table"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.CodeIndexSnapshots', N'U') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Builder model configuration evidence table"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.BuilderModelConfigurationRecords', N'U') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Project technical readiness companion table"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.ProjectTechnicalReadinessEvidence', N'U') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Effective Workbench readiness view"; Sql = "SELECT CASE WHEN OBJECT_ID(N'dbo.vw_WorkbenchEffectiveProjectReadiness', N'V') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Technical validation exact fence"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.TechnicalValidationAttempts') AND name=N'FK_TechnicalValidationAttempts_Fence' AND is_not_trusted=0" }
             ,@{ Name = "Technical validation exact ClientOperation"; Sql = "SELECT COUNT(*) FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.TechnicalValidationAttempts') AND name=N'FK_TechnicalValidationAttempts_ClientOperationAuthority' AND is_not_trusted=0" }
             ,@{ Name = "Technical validation one running per project"; Sql = "SELECT COUNT(*) FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.TechnicalValidationAttempts') AND name=N'UX_TechnicalValidationAttempts_OneRunningPerProject' AND is_unique=1 AND has_filter=1" }
             ,@{ Name = "Technical validation ClientOperation result columns"; Sql = "SELECT CASE WHEN COL_LENGTH(N'dbo.ClientOperations', N'ResultTechnicalValidationAttemptId') IS NULL OR COL_LENGTH(N'dbo.ClientOperations', N'ResultProjectTechnicalReadinessEvidenceId') IS NULL OR COL_LENGTH(N'dbo.ClientOperations', N'ResultProjectReadinessAssessmentId') IS NULL THEN 0 ELSE 1 END" }
             ,@{ Name = "Technical validation ClientOperation result authority"; Sql = "SELECT CASE WHEN COUNT(*)=2 THEN 1 ELSE 0 END FROM sys.foreign_keys WHERE parent_object_id=OBJECT_ID(N'dbo.ClientOperations') AND name IN (N'FK_ClientOperations_TechnicalValidationAttemptAuthority', N'FK_ClientOperations_TechnicalReadinessEvidenceAuthority') AND is_not_trusted=0" }
             ,@{ Name = "Technical readiness canonical nine gates"; Sql = "SELECT CASE WHEN COL_LENGTH(N'dbo.ProjectTechnicalReadinessEvidence', N'GateResultsJson') IS NOT NULL AND COL_LENGTH(N'dbo.ProjectTechnicalReadinessEvidence', N'GateResultsSha256') IS NOT NULL AND EXISTS (SELECT 1 FROM sys.check_constraints WHERE parent_object_id=OBJECT_ID(N'dbo.ProjectTechnicalReadinessEvidence') AND name=N'CK_ProjectTechnicalReadinessEvidence_Gates' AND is_not_trusted=0) THEN 1 ELSE 0 END" }
             ,@{ Name = "Technical evidence append-only triggers"; Sql = "SELECT CASE WHEN COUNT(*)=6 THEN 1 ELSE 0 END FROM sys.triggers WHERE parent_id IN (OBJECT_ID(N'dbo.RepositoryStateObservations'), OBJECT_ID(N'dbo.BuildValidationRecords'), OBJECT_ID(N'dbo.TestValidationRecords'), OBJECT_ID(N'dbo.CodeIndexSnapshots'), OBJECT_ID(N'dbo.BuilderModelConfigurationRecords'), OBJECT_ID(N'dbo.ProjectTechnicalReadinessEvidence')) AND name IN (N'TR_RepositoryStateObservations_AppendOnly', N'TR_BuildValidationRecords_AppendOnly', N'TR_TestValidationRecords_AppendOnly', N'TR_CodeIndexSnapshots_AppendOnly', N'TR_BuilderModelConfigurationRecords_AppendOnly', N'TR_ProjectTechnicalReadinessEvidence_AppendOnly') AND is_disabled=0" }
         )
