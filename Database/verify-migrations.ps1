...
try {
    ...
    try {
        $checks = @(
            ...
            ,@{ Name = "PatchArtifact expiry check constraint"; Sql = "SELECT COUNT(*) FROM sys.check_constraints WHERE parent_object_id = OBJECT_ID(N'governance.PatchArtifact') AND name = N'CK_PatchArtifact_ExpiresAfterCreated'" }            ,@{ Name = "memory.MemoryProposal table"; Sql = "SELECT CASE WHEN OBJECT_ID(N'memory.MemoryProposal', N'U') IS NULL THEN 0 ELSE 1 END" }
