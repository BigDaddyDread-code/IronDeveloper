...
try {
    ...
    try {
        $checks = @(
            ...
            ,@{ Name = "WorkflowTransitionRecord truth table check constraint"; Sql = "SELECT COUNT(*) FROM sys.check_constraints WHERE parent_object_id = OBJECT_ID(N'governance.WorkflowTransitionRecord') AND name = N'CK_WorkflowTransitionRecord_ContinueTruth'" }            ,@{ Name = "memory.MemoryProposal table"; Sql = "SELECT CASE WHEN OBJECT_ID(N'memory.MemoryProposal', N'U') IS NULL THEN 0 ELSE 1 END" }
