...
public sealed class AgentHandoffContractTests
{
    ...
    public void AgentHandoff_StaticBoundary_IsNotWiredIntoApiCliRuntimeSqlWorkflowOrA2a()
    {
        ...
        var files = Directory
            ...
            .Where(path => !path.Contains($"{Path.DirectorySeparatorChar}IronDev.IntegrationTests.Api{Path.DirectorySeparatorChar}", StringComparison.OrdinalIgnoreCase))
            ...
            .Where(path => !path.EndsWith(Path.Combine("IronDev.Core", "Agents", "GroundingEvidenceReferenceModels.cs"), StringComparison.OrdinalIgnoreCase))
            .Where(path => !path.EndsWith(Path.Combine("IronDev.Core", "Agents", "ThoughtLedgerHandoffEntryModels.cs"), StringComparison.OrdinalIgnoreCase))
