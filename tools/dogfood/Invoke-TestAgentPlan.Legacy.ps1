...
if ($env:IRONDEV_SKIP_LEGACY_RUNNER_BUILD -eq "1") {
    Set-Content -LiteralPath $buildLogPath -Value "Skipped runner build because C# TestPlanRunner invoked the legacy compatibility path from a running ReplayRunner process." -Encoding UTF8
} else {
        ...
        switch ($action) {
            ...
            "builder_repair_loop_smoke" {
                ...
                if ($exitCode -ne 0) {
                    $status = "FAILED"
                    $summary = if ($parsed) {
                        "Builder repair loop failed; passed=$($parsed.passed); trace=$($parsed.traceId)"
                    } else {
                        "builder_repair_loop_smoke exited with code $exitCode"
                    }
                } elseif ($parsed -and -not [bool]$parsed.passed) {
                    $status = "FAILED"
                    $summary = "Builder repair loop returned Passed=false"
                } else {
                    $summary = "Builder repair loop passed; trace=$($parsed.traceId); repairs=$(@($parsed.trace.repairAttempts).Count); recommendation=$($parsed.report.recommendation)"
                }
                ...
                if ($status -eq "SUCCESS" -and $parsed) {
                    ...
                    foreach ($classification in @($params.expect_build_failure_classifications)) {
                        if ($classification -and @($parsed.trace.buildAttempts | Where-Object { [string]$_.failureClassification -eq [string]$classification }).Count -eq 0) {
                            $validationFailures.Add("Expected build failure classification '$classification'.") | Out-Null
                        }
                    }
                    foreach ($classification in @($params.expect_test_failure_classifications)) {
                        if ($classification -and @($parsed.trace.testAttempts | Where-Object { [string]$_.failureClassification -eq [string]$classification }).Count -eq 0) {
                            $validationFailures.Add("Expected test failure classification '$classification'.") | Out-Null
                        }
                    }
                    if ($params.expect_build_failure -and @($parsed.trace.buildAttempts | Where-Object { [string]$_.status -eq "Failed" }).Count -eq 0) {
                        $validationFailures.Add("Expected at least one failed build attempt.") | Out-Null
                    ...
                    if ($params.expect_final_build_success -and @($parsed.trace.buildAttempts | Where-Object { [string]$_.status -eq "Succeeded" }).Count -eq 0) {
                        $validationFailures.Add("Expected at least one successful build attempt.") | Out-Null
                    }
                    if ($params.expect_final_test_success -and @($parsed.trace.testAttempts | Where-Object { [string]$_.status -eq "Succeeded" }).Count -eq 0) {
                        $validationFailures.Add("Expected at least one successful test attempt.") | Out-Null
                    }
                    if ($params.expect_repair_attempts_min -and @($parsed.trace.repairAttempts).Count -lt [int]$params.expect_repair_attempts_min) {
                        $validationFailures.Add("Expected at least $($params.expect_repair_attempts_min) repair attempts.") | Out-Null
