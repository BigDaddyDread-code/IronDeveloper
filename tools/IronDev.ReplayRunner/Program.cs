...
if (args.Length >= 3 &&
    string.Equals(args[0], "agent", StringComparison.OrdinalIgnoreCase) &&
    string.Equals(args[1], "sentinel", StringComparison.OrdinalIgnoreCase) &&
    string.Equals(args[2], "observe", StringComparison.OrdinalIgnoreCase))
{
...
static async Task<int> HandleAgentSupervisorRunGoalCommandAsync(string[] args, JsonSerializerOptions options)
{
    ...
    {
        Console.Error.WriteLine("Usage: IronDev.ReplayRunner agent supervisor run-goal --project <project> --query <query> --plan <path> [--run-id id] [--live-llm] [--model-profile profile] [--json]");
        return 2;
    }
    ...
    var (_, _, runner) = CreateAgentRuntime(enableLiveLlm: liveLlm, supervisorModelProfile: modelProfile);
    var result = await runner.RunAsync(new AgentRequest
    {
...
static async Task<int> HandleAgentQualityRunGateCommandAsync(string[] args, JsonSerializerOptions options)
{
    ...
    var (_, _, runner) = CreateAgentRuntime(enableLiveLlm: liveLlm, qualityModelProfile: modelProfile);
    var result = await runner.RunAsync(new AgentRequest
    {
...
static async Task<int> HandleAgentSentinelObserveCommandAsync(string[] args, JsonSerializerOptions options)
{
    ...
    var affectedProject = ReadOption(args, "--affected-project") ?? "IronDev";
    var findingType = ReadOption(args, "--finding-type") ?? "RoutingWeakness";
    ...
    Console.WriteLine(JsonSerializer.Serialize(new
    {
        command = "agent sentinel observe",
        agent = result.AgentName,
        status = result.Status.ToString(),
        summary = result.Summary,
        modelProfile = result.ModelProfileName,
        ...
        model = result.Model,
        exitCode = result.ExitCode,
        insight = TryParseJson(result.OutputJson),
        completedAtUtc = result.CompletedAtUtc
    }, options));
...
static async Task<int> HandleAgentResearchPackageCommandAsync(string[] args, JsonSerializerOptions options)
{
    ...
    {
        Console.Error.WriteLine("Usage: IronDev.ReplayRunner agent research package --project <project> --topic <topic> [--source-url url] [--source-title title] [--source-type type] [--snippet text] [--run-id id] [--live-llm] [--model-profile profile] [--json]");
        return 2;
    }
    ...
    var (_, _, runner) = CreateAgentRuntime(enableLiveLlm: liveLlm, researchModelProfile: modelProfile);
    var result = await runner.RunAsync(new AgentRequest
    {
        ...
        .Select<AgentDefinition, IIronDevAgent>(definition =>
            string.Equals(definition.Name, "TesterAgent", StringComparison.OrdinalIgnoreCase)
                ...
                : string.Equals(definition.Name, "ResearchAgent", StringComparison.OrdinalIgnoreCase)
                    ? new ResearchAgent(definition, modelResolver, agentLlmClient)
