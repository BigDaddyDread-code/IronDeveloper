...
public static class IronDevCliMemoryImprovements
{
    ...
    public static bool IsMemoryImprovementsCommand(string[] args) =>
        args.Length >= 1 && string.Equals(args[0], "memory-improvements", StringComparison.OrdinalIgnoreCase);
    ...
    {
        if (args.Length < 2)
            return WriteUsageError("memory-improvements", output, error, IsJsonOutput(args, environment), "Missing memory-improvements subcommand.");
        ...
        return args[1].ToLowerInvariant() switch
        {
            ...
            _ => WriteUsageError("memory-improvements", output, error, IsJsonOutput(args, environment), $"Unknown memory-improvements subcommand: {args[1]}.")
        };
    }
    ...
    private static async Task<int> HandleCreateAsync(
        ...
        CancellationToken cancellationToken)
    {
        var resolved = ResolveRequiredOptions("memory-improvements create", args, output, error, environment);
        ...
        if (HasUnsupportedAuthorityFlag(args, out var unsupportedFlag))
            return WriteUsageError("memory-improvements create", output, error, IsJson(resolved.Options.Output), $"Unsupported authority, promotion, or execution flag for memory-improvements create: {unsupportedFlag}.");

        if (!TryGetIntOption(args, "--project-id", out var projectId))
            return WriteUsageError("memory-improvements create", output, error, IsJson(resolved.Options.Output), "Missing or invalid required option: --project-id <id>.");

        var targetAgentRunId = GetOption(args, "--target-agent-run-id");
        if (string.IsNullOrWhiteSpace(targetAgentRunId))
            return WriteUsageError("memory-improvements create", output, error, IsJson(resolved.Options.Output), "Missing required option: --target-agent-run-id <id>.");

        var request = new ManualMemoryImprovementCreateRequest
        {
            ProjectId = projectId,
        ...
        return await CallApiAsync(
            ...
            error,
            handler,
            client => client.CreateManualMemoryImprovementAsync(request, cancellationToken),
    ...
    private static async Task<int> HandleGetAsync(
        ...
        CancellationToken cancellationToken)
    {
        var resolved = ResolveRequiredOptions("memory-improvements get", args, output, error, environment);
        if (resolved.ExitCode is not null)
            return resolved.ExitCode.Value;

        if (!TryGetPathArgument(args, 2, out var agentRunId))
            return WriteUsageError("memory-improvements get", output, error, IsJson(resolved.Options.Output), "Missing required argument: <agentRunId>.");

        if (!TryGetIntOption(args, "--project-id", out var projectId))
            return WriteUsageError("memory-improvements get", output, error, IsJson(resolved.Options.Output), "Missing or invalid required option: --project-id <id>.");
        ...
        return await CallApiAsync(
            ...
            error,
            handler,
            client => client.GetManualMemoryImprovementAsync(projectId, agentRunId, cancellationToken),
    ...
    private static void WriteCreateText(JsonElement root, TextWriter output, int projectId, string targetAgentRunId)
    {
        ...
        output.WriteLine("Memory improvement proposal requested");
        output.WriteLine($"  Project: {projectId}");
        output.WriteLine($"  Target agent run: {Safe(targetAgentRunId)}");
        output.WriteLine($"  Proposal run: {Safe(GetString(root, "runId", GetString(data, "agentRunId", "<unknown>")))}");
        output.WriteLine($"  Proposal id: {Safe(GetString(root, "proposalId", "<none>"))}");
        output.WriteLine($"  Status: {Safe(GetString(root, "status", "<unknown>"))}");
        output.WriteLine($"  Pattern count: {GetInt(data, "patternCount")}");
        output.WriteLine($"  Proposal count: {GetInt(data, "proposalCount")}");
    ...
    {
        var data = GetObject(root, "data");
        output.WriteLine("Memory improvement proposal");
        output.WriteLine($"  Project: {projectId}");
        output.WriteLine($"  Agent run: {Safe(agentRunId)}");
        output.WriteLine($"  Detection result: {Safe(GetString(data, "detectionResultId", "<unknown>"))}");
        ...
        output.WriteLine($"  Proposal ids: {GetArrayLength(data, "proposalIds")}");
        output.WriteLine($"  Evidence refs: {GetArrayLength(data, "evidenceRefs")}");
        output.WriteLine($"  Proposal-only output: {GetBool(data, "proposalOnlyOutput").ToString().ToLowerInvariant()}");
    ...
    private static void WriteBoundaryText(JsonElement root, TextWriter output)
    {
        ...
        output.WriteLine($"    Memory proposal is promotion: {GetBool(boundary, "memoryProposalIsPromotion").ToString().ToLowerInvariant()}");
        output.WriteLine($"    Memory safe is approval: {GetBool(boundary, "memorySafeIsApproval").ToString().ToLowerInvariant()}");
        output.WriteLine($"    Candidate is memory: {GetBool(boundary, "candidateIsMemory").ToString().ToLowerInvariant()}");
        output.WriteLine($"    Retrieval match is memory candidate: {GetBool(boundary, "retrievalMatchIsMemoryCandidate").ToString().ToLowerInvariant()}");
        ...
        output.WriteLine($"    Collective memory written: {GetBool(boundary, "collectiveMemoryWritten").ToString().ToLowerInvariant()}");
        output.WriteLine($"    Vector authority written: {GetBool(boundary, "vectorAuthorityWritten").ToString().ToLowerInvariant()}");
        output.WriteLine($"    Tool executed: {GetBool(boundary, "toolExecuted").ToString().ToLowerInvariant()}");
        output.WriteLine("    Memory proposal is not promotion.");
        output.WriteLine("    Memory safe is not approval.");
        output.WriteLine("    Candidate is not memory.");
    }
