...
PR-02C-A makes the V2 Workshop composer an AgentRun client when `WorkbenchV2:ConversationAuthorityEnabled` is explicitly true. The shell performs a no-chat readiness preflight, creates or reuses the chat session through a durable idempotent operation, submits one fenced AgentRun command, polls backend state, recovers the active and latest terminal run after reload, requests cancellation through its own idempotent operation, and reloads server-owned messages after terminal materialization. It does not call `/chat/complete` or write user/assistant messages directly. New-session lease validation/renewal, client-operation creation, and session insertion share one serializable transaction; a stale first-create fence therefore writes neither the session nor its operation ledger row, while a completed ambiguous retry replays before fencing. The backend rejects compatibility mutations plus legacy session update/delete while the flag is active, enforces active project membership and project scope on parent and child chat routes, and rejects unavailable worker or deterministic provider configuration before any first-session or turn write. Late client responses are generation-fenced to the exact project, Workbench session, and lease so navigation cannot install an old run in a new context. The V1/compatibility path remains unchanged while the flag is false.

The first accepted AgentRun still binds the active Workbench session to exactly one direct chat session. The V2 shell fences new/switch-direct-conversation controls while submission is in flight, while a create or submit response remains ambiguous, and after the binding. Ambiguous receipts are retained by full authority/operation key; only the unchanged message may replay them, and recovery does not silently discard or reinterpret the receipt. Reopening the project resumes the same Workbench session and binding; this preview intentionally has no close/new-Workbench-session action, so it does not claim that reopening starts a different direct conversation. Document attachments, structured BA draft metadata, durable understanding updates, and `/ticket` formalization are not silently synthesized by this slice and remain later contracts.
