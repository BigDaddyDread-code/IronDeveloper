...
The hosted outbox worker remains explicitly feature-gated outside isolated environments. The PR-02B LocalTest preview enables it with an explicit context-aware deterministic Analyst provider; that provider is test-only and is never a fallback for an unknown or failed production provider. Unverifiable legacy assistant rows are display history only and are excluded from provider context rather than misattributed to the user. The current Workshop chat UI remains on its compatibility path until the later conversation slice switches submission and polling to this authority.
