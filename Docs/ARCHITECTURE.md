...
PR-04B adds an explicit deterministic commit service, separate from the BA path. It accepts only the exact current `Ready` proposal revision while the project is `Shaping`, every question/conflict is resolved, and the current Workbench lease is valid. One serializable transaction creates the permanent `ProjectTickets`, matching `WorkItems` and immutable first contracts, exact source-message references, proposal-to-ticket/dependency mappings, an append-only commitment receipt, a `Committed` proposal revision, the `Delivery` lifecycle revision, the client-operation result, attribution, and outbox evidence. Exact replay returns the original ticket IDs; a changed payload, stale fence/revision, second operation, `NeedsInput` set, or open issue creates nothing. This transition does not configure readiness, bind a repository, touch the filesystem, or grant Builder execution authority.

PR-05A makes repository setup a distinct Workbench authority rather than an extension of project creation or legacy provisioning readiness. A read-only planner combines the canonical server-owned project name, latest typed Project Understanding, one explicitly selected pinned profile, and a server-configured approved repository root. It derives safe identifiers and a direct-child target, rejects unsafe or existing paths, reports unsupported desired technology without substitution, and returns a canonical hash-bound plan covering profile metadata, generated names, template hash, toolchain/image, exact commands, Git initialization, indexing, sandbox validation, and resource policy. Confirmation accepts only the current Workbench session/lease, a client operation ID, and that exact plan hash; it recomputes the plan and atomically records versioned `RepositoryBinding` and `ProjectExecutionProfile` authorities, an immutable confirmation receipt, a `NotConfigured` readiness assessment, attribution, operation result, and outbox evidence. It stops at `SetupConfirmed`: `Project.LocalPath` is not projected, no target or staging directory is created, no template/Git/index/command/sandbox action runs, and no Builder authorization exists. Legacy `LocalPath` values are represented only as `LegacyUnverified` bindings. PR-05B owns consuming a confirmed receipt to provision a product-neutral shell.

PR-05B consumes that immutable setup authority through one separately confirmed, fenced, and idempotent provisioning operation. The product request is deliberately smaller than the recorded plan: current Workbench session/lease, one client operation ID, setup-confirmation ID, and expected repository-binding/profile revisions only. The service revalidates the configured approved root and exact stored plan/profile hashes, claims a durable attempt, renders only the server-pinned product-neutral bundle into a direct sibling staging directory, initializes a controlled offline Git repository on `main`, and installs it with a same-volume atomic move. It never adopts or overwrites an unknown target. Commit trailers and the pinned manifest support recovery after an ambiguous post-move interruption without creating a different baseline. Finalization advances the versioned binding to `Qualified`, records the default branch and immutable baseline, and projects `Project.LocalPath` only from that authority. `Qualified` here means repository artifact integrity only: execution readiness remains `NotConfigured`, and no restore, build, test, index, sandbox validation, BA/model call, or Builder authorization occurs. PR-06 owns technical qualification.

PR-06A adds the production sandbox boundary without yet asserting technical readiness. A fenced, idempotent qualification operation binds one durable attempt to the exact qualified repository baseline, repository-binding revision, execution-profile revision, descriptor/template/toolchain hashes, digest-pinned Windows SDK image, content-addressed offline feed, and immutable resource-policy hash. The runtime uses the local Windows container engine to create an HCS-managed Hyper-V-isolated container with no network endpoint, exactly three read-only host inputs, a bounded writable layer, fixed restore/build/test commands, and an explicitly reduced credential-free environment. It inspects the applied image, isolation, network, mounts, limits, and restart policy before project code runs. The Windows-specific process guarantee is `MaximumUntrustedWorkloadProcessCount = 64`: a fixed versioned and hash-bound supervisor creates every project process suspended, assigns it to an unnamed non-breakaway Job Object with `ActiveProcessLimit = 64` and kill-on-close, and resumes it only after assignment. HCS, container-bootstrap, and supervisor processes are trusted boundary machinery outside that workload count; IronDev does not claim that Docker applies a whole-silo PID limit. Failure to set/query the job, assign the suspended probe, establish the restricted workload identity, or pass broker-denial probes makes capability unavailable before project bytes are read. The runtime copies only bounded sanitized evidence and removes only exact ownership-labelled resources on completion or replay. Missing or drifting controls fail closed and never fall back to host project execution. Evidence manifests and cleanup outcomes are durable and append-only. A bounded recovery worker serializes with live work through the same project application lock, materializes only exact completed evidence, never reruns an existing pending operation, and leaves the attempt fenced as `Running` when teardown cannot be proven. Passing PR-06A evidence still leaves execution readiness `NotConfigured` and grants no Builder authorization. PR-06B owns current validation, indexing, and readiness projection. ADR-018 records the process-limit scope and release proof gate.

PR-06B turns exact technical evidence into a separate, permission-free readiness authority. A fenced, idempotent server operation observes the qualified repository with fixed Git commands, records its baseline/head/cleanliness and fingerprint, materializes restore/build/test facts only from a verified PR-06A sandbox manifest, and creates an immutable file-level code-index snapshot from the exact clean Git tree. Every record repeats the repository-binding and execution-profile IDs/revisions, baseline, observation and fingerprint, command hashes, toolchain/image/policy/feed/template identities, validation timestamp, and evidence-manifest hash. The currentness evaluator compares those content and revision values to current authority; timestamps are descriptive and cannot make mismatched evidence current. The nine gates are repository binding qualified, clean at baseline, pinned profile, restore passed, build passed, test command passed, code index current, sandbox qualified, and stable Builder model configured. Without a qualified binding and pinned profile, a project is `NotConfigured`; configured but missing, failing, or stale evidence projects as `ValidationRequired`; only nine current passing gates project as `Ready`. `Project.LastIndexedUtc`, `IndexingStatus`, and `IndexedFileCount` remain one-way compatibility projections. Live provider/queue availability is checked separately and never changes durable readiness. PR-06B creates no Builder work package, authorization, run, or source-mutation authority.

PR-07A creates authorization only. A server-owned immutable `BuilderWorkPackage` freezes the tenant, project, Work Item, permanent ticket, current Work Item contract revision/hash, acceptance criteria, and explicit permitted-file scope together with the repository binding/baseline/branch, exact readiness assessment and evidence hash, repository observation, complete file-level code-index snapshot, pinned Builder role/prompt/tool/context/output contract versions, effective execution and model profiles, and qualified sandbox policy/image/evidence identities. Canonical JSON and SHA-256 bind the whole authorization-free core; a separate fifteen-minute, single-use authorization envelope binds its exact ID and hash. Creation and grant are fenced, idempotent, append-only, and revalidate current membership, lease, lifecycle, ticket contract, branch/baseline, readiness, index, model profile, and sandbox authority. The client supplies only selected permanent ticket IDs and expected package hash; it cannot supply paths, prompts, tools, models, policies, images, or additional context. PR-07A never creates an AgentRun, invokes a provider, enters a sandbox, or writes repository source. PR-07B owns atomic revalidation, authorization consumption, durable Builder AgentRun creation, effective-profile freeze, role-context materialization, and prompt/tool/input hashing before any provider call.

PR-07B is the atomic consume-and-prepare boundary. The server observes the bound branch and head, opens a serializable transaction, locks the exact authorization and immutable work-package core, revalidates current repository/readiness/model/sandbox and Work Item authority, freezes the effective Builder profile, materializes the code-owned Builder prompt, exact role context, declared tool manifest, and provider input, and records every content hash in one durable `BuilderAgentRun`. Only after the run insert succeeds does the same transaction consume the authorization and mark provider invocation permitted. The prepared run remains `Prepared` with `ProviderInvokedAtUtc = NULL`; this slice has no provider gateway dependency, executes no tool, enters no sandbox, and writes no repository. PR-07C owns provider invocation and qualified sandbox execution.
