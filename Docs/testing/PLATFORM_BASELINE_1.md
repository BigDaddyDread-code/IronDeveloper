...
The full SQL lane invokes the isolated platform baseline before its existing store lanes. The baseline creates, migrates, tests, and removes a separate catalog, so legacy store fixtures cannot alter or be altered by the clean-room schema. Process-executing endpoint scenarios remain in the governed-loop lanes; they are not part of the deterministic API/database baseline. The frontend workflow runs the live Swagger regeneration and dirty-tree gate. Together they reject migration, API, generated-contract, or TypeScript drift without requiring a developer stack.
