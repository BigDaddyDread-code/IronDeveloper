...
## REL-4 Live Model Ticket Draft Smoke

- Added focused `AlphaSmoke`, `ReleaseReadiness`, and `RequiresExternalDependency` metadata to the live model ticket-draft smoke test.
- REL-4 does not add `RequiresRealDatabase` or `LongRunning`; it requires explicit live-provider opt-in and writes bounded draft evidence only.
- REL-4 verifies live model output can produce a bounded BookSeller ticket draft with the expected Book.cs file reference.
- REL-4 does not persist a ticket, start a run, request critic review, create approval, continue workflow, apply source, commit, push, release, or deploy.
- Live model draft evidence is useful. It is not ticket persistence, approval, source authority, or release readiness.
...
- DEMO-5 verifies check-only behavior, root-safety blocking, loopback-only API/UI targets, SQL check-only delegation, guarded API/UI start-or-verify markers, guarded seed delegation, app URL reporting, and one next-safe-action output.
- DEMO-6 verifies deterministic-only local alpha banner text, no silent live-to-deterministic fallback, visible flow-shell model mode, and blocked live-model requests that do not start services.
...
## DEMO-7 / DEMO-8 / DEMO-9 Dogfood Gate Artifacts

- Added focused `DemoRehearsal`, `DogfoodGate`, and `BookSellerBatch` metadata to the dogfood artifact contract test.
- Added broad `AlphaSmoke`, `ReleaseReadiness`, `Contract`, and `Boundary` metadata to the same contract test.
- DEMO-7 records the non-author rehearsal transcript shape and current `Blocked` verdict.
- DEMO-8 records DOGFOOD-ALPHA-LOCAL-001 as `EvidenceIncomplete` instead of inventing release readiness.
- DEMO-9 records the optional three-ticket BookSeller batch as `BatchEvidenceIncomplete` and parks all three tickets without fake run/approval/apply/report refs.
- Dogfood artifacts are evidence only. They do not approve, release, deploy, publish, tag, merge, continue workflow, apply source, or replace backend truth.
