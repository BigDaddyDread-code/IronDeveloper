...
- `IronDev.Client` includes product-shaped run status/report/event methods mapped to `/api/runs/*`; creation/lifecycle is still missing, and events are currently report-backed snapshots rather than live durable events.

## Leaking Or Missing

- Current Product CLI ticket commands are now API/client-boundary clean; they use `IIronDevApiClient`.
- Product CLI has four ticket commands plus run status/report/stream commands.
...
| Evidence area | Count |
|---|---:|
| Actual IronDev.Api routes | 87 |
| Planned endpoint gaps documented | 11 |
| HTTP-backed typed client methods, excluding overlapping product facade | 84 |
| Product `IIronDevApiClient` facade methods | 13 |
...
|---|---|---|---|
| Product CLI surface is incomplete. | Ticket and run status/report/stream commands exist; projects, documents, memory, generation, and build-run commands are missing. | High | Add API/client-backed project, document, memory, generation, and build commands. |
| Durable run lifecycle is incomplete. | `/api/runs/{runId}`, `/api/runs/{runId}/report`, and `/api/runs/{runId}/events` exist over report storage, but run creation and live events do not. | High | Add run starters and a durable event store behind the SSE endpoint. |
