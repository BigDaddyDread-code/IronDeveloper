...
| `workspace apply-copy` | retired source-mutation entry point | command arguments only | none | No | No | N/A; command fails closed | No | Always returns `ProjectApplyCapabilityContextMissing`; source mutation is owned by the injected governed project-work spine. |
...
- No standalone CLI workspace command may mutate the source repository.
- Controlled add/modify mutation is owned by the injected governed project-work spine and its `IControlledSourceMutationExecutor`.
...
- Agents must not bypass the governed project-work spine or create their own source mutation path.
