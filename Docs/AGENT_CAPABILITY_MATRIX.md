...
- Unsafe/not allowed: intentionally blocked by the current governance boundary.
...
| ArchitectAgent | Opt-in live governed reviewer | Reviews proposals against weighted context and safety boundaries, with deterministic fallback. | Advisory only; it cannot create an accepted decision, patch source, or grant authority. |
| BuilderAgent | Trace-backed disposable repair loop | Builder path has disposable smoke coverage, a traceable internal spine, and a first real caged repair loop for Solitaire with intentional build/test failures and bounded repairs. | Writes remain limited to explicit disposable workspaces. No active-repository writes, memory mutation, guardrail mutation, or self-approval. |
...
| MemoryImprovementAgent | Level 1 proposal-only memory reviewer | Reads focused completed-run evidence and proposes memory improvements with evidence bundles, token/proposal budgets, and a MemoryKeyGate review. | Proposal only. It cannot write staging memory or accepted memory. Accepted-memory key readiness is not granted; no ticket creation, patching, or self-approval. |
...
The agent layer provides governed planning, evidence gathering, disposable execution, review, and bounded repair. It is not an unsupervised autonomous developer.

Product runs and agent capabilities are related but not interchangeable. A backend workflow may execute a governed stage or controlled apply only through its explicit policy, evidence, and approval contracts; the participating agent does not acquire that authority.

## Boundary

No agent currently has permission to mutate the active project repository autonomously. Approved changes may be applied by the separate governed skeleton or controlled-worktree services into disposable or isolated workspaces; that is backend authority, not agent self-authority.
