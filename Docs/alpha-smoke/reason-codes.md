...
| `TicketPersisted` | REL-3 created the BookSeller project and ticket through the authenticated API backed by SQL. | SQL/API persisted smoke is running. | Continue only through the remaining governed smoke stages. | Treat ticket persistence as run success. |
