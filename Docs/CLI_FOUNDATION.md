...
Tool request commands are API clients over the PR61 Tool Request API. They may create and inspect request forms. They do not approve requests, execute tools, evaluate gates, apply source, promote memory, append tool execution audit, or expose hidden reasoning. Tool requests created through the CLI are durable SQL-backed request records when the API is backed by the durable Tool Request Store, but they are still not execution evidence.
