...
`Ai__Provider` must explicitly select `Fake`, `OpenAI`, `LocalOpenAI`, `Ollama`, or `Custom`; missing and unknown values fail instead of taking runtime defaults. `OpenAI` requires `Ai__ApiKey` or `OPENAI_API_KEY`; `LocalOpenAI`, `Ollama`, and `Custom` require `Ai__BaseUrl`. When `Weaviate__Enabled=true`, `Weaviate__Endpoint` is required, and a non-boolean enabled value is rejected.
