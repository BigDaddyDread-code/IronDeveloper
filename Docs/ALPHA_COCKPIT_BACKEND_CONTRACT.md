...
- PR-03 returns a deterministic `/ticket` routing receipt but creates no proposal or permanent ticket. The separate PR-04A aggregate owns proposal generation and review. The legacy AgentRun submit endpoint rejects any leading-slash candidate with `400 workbench_input_route_required` so command handling cannot be bypassed.
