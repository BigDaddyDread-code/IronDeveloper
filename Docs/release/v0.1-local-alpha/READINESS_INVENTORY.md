...
| chat-to-ticket | Proven to gate | REL-5 persists a chat session/message, classifies the turn as formalization, confirms a draft ticket with chat provenance, starts the skeleton run through the API, and halts at `PausedForApproval` with SQL/run evidence. | Product UI path and approval/continuation/apply from a chat-created ticket remain separate. |
