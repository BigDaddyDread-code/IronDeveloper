...
| root safety | Partially proven | J10 root-safety validator exists and disposable workspace execution uses root validation before creating workspaces/evidence/running commands; alpha smoke has local output-root safety. | Release-wide root safety is not yet a single invoked gate across doctor, smoke, setup, evidence, workspace, and apply paths. |
