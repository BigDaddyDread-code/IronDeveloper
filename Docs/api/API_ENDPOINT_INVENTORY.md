...
| POST | `/api/projects/{projectId}/tickets/draft/confirm` | `TicketsController.ConfirmDraft` | Persist a confirmed draft ticket with source provenance. | Implemented | TauriShell, API smoke | Persists ticket only; does not start a run or grant approval. |
